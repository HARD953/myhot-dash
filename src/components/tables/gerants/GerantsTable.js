  ...
  const actionsBody = (rowdata) => {
    return (
      <section className="d-flex justify-content-between align-items-center gap-2">
        <Button
          label="Consulter"
          onClick={() => getGerantById(rowdata?.id)}
          className="rounded"
        />
        <Button
          label="Editer"
          onClick={() => showUpdateDialogGerant(rowdata?.id)}
          className="rounded p-button-info"
        />
        <Button
          onClick={() => deleteGerantConfirm(rowdata?.id)}
          label="Supprimer"
